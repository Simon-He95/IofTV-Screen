...
<script setup>
  ...
  import { ref } from 'vue'
  const maptitle = ref("设备分布图");
  const options = ref({});
  const code = ref("china"); //china 代表中国 其他地市是行政编码
  const echartBindClick = ref(false);
  const isSouthChinaSea = ref(false); //是否要展示南海群岛  修改此值请刷新页面
  ...
  function getData(code) {
    currentGET("big8", { regionCode: code }).then((res) => {
      ...
      if (res.success) {
        getGeojson(res.data.regionCode, res.data.dataList);
        mapclick();
      } else {
        message(res.msg);
      }
  ...
  async function getGeojson(name, mydata) {
    code.value = name;
    //如果要展示南海群岛并且展示的是中国的话
    let geoname = name
    if (isSouthChinaSea.value && name == "china") {
      geoname = "chinaNanhai";
    }
    //如果有注册地图的话就不用再注册 了
  ...
  function mapclick() {
    if (echartBindClick.value) return;
    //单击切换到级地图，当mapCode有值,说明可以切换到下级地图
    centerMap.value.chart.on("click", (params) => {
      // console.log(params);
